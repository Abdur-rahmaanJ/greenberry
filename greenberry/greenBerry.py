        ...
        #
            ...
            try:
                ...
                if g_fs[func_name]['params'] is None:
                    ...
                    param_vals = GreenBerrySearch.search_toks(i, 1, words, [S.NL, S.EOF])
                    registry = g_fs[func_name]['params']
                    i = 0
                    for key in registry:
                        registry[key] = [param_vals[i], GreenBerryVarType.var_type(param_vals[i])]  # data
                        i += 1
                ...
                if words[i+1] in g_cls:
                    if words[i+2] == S.ATTRIB:
                        if words[i+4] == S.EQUAL:
                            value = GreenBerryVarType.var_data(i+4, words, [S.NL, S.EOF])
                        ...
                        if words[i+4] == S.COLON:
                            g_cls[words[i+1]]['actions'][words[i+3]] = GreenBerrySearch.search(i, 4, words, [S.NL, S.EOF])
                        else:
                            print(E.COLON, line)
